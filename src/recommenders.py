...
class MainRecommender:
    ...
    def __init__(self, data: pd.DataFrame, weighting: bool = True):
        ...
        if weighting:
            self.user_item_matrix = bm25_weight(self.user_item_matrix.T, B=0.1, K1=5).T
    ...
    @staticmethod
    def fit(user_item_matrix, n_factors=100, regularization=0.04, iterations=20, num_threads=4):
        """Обучает ALS"""
    ...
    def _get_scores(self, user, model, N=5):
        ...
        self._update_dict(user_id=user)
        scores_rec = [rec[1] for rec in model.recommend(userid=self.userid_to_id[user],
        ...
        if len(scores_rec) < N:
            overall_top_purchases_scores = [item_score[1] for item_score in 
                                            model.rank_items(userid=self.userid_to_id[user],
                                                             user_items=csr_matrix(self.user_item_matrix).tocsr(),
                                                             selected_items=[self.itemid_to_id[item] for item in 
                                                                             self.overall_top_purchases[:N]],
                                                             recalculate_user=True)]
            scores_rec.extend(overall_top_purchases_scores)
            scores_rec = scores_rec[:N]
